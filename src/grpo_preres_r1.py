...
from open_r1.trainer.grpo_trainer import VLMGRPOTrainer, GRPOConfig
...
def accuracy_reward(completions, solution, **kwargs):
    ...
    lambda_std = float(kwargs.get("lambda_std", 0.5))      # 标准差惩罚权重
    flat_solutions = solution  # 传入的 solution 列表
    reshaped_solution = []  
    ...
    for i in range(len(reshaped_content)):
        ...
        for triple in sample_triples:
            ...
            for elem_dim in range(num_element_dims):
                ...
                if abs(right_val - centroid_val) < 0.05:
                    gain[right_sample] += 1
        max_possible_gain = sample_triple_count * num_element_dims 
        reward = torch.where(
            max_possible_gain > 0,  
        ...
        batch_triple_compare.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            with open(log_path, "a", encoding='utf-8') as f:
                for j in range(K):
                    ...
                    f.write(f"------------- {current_time} raw_pairwise: {original_order_rewards[j]}-------------\n")
                    # f.write(f"------------- {current_time} final_triplet_rewards: {final_triplet_rewards[j]}-------------\n")
                    
                    # f.write(f"gaussian_penalty: {gaussian_penalty}, lambda_human: {lambda_human}\n")
                    if img_block:
                        try:
    ...
    temp_tensor_list = []
    sorted_indices_list = []
    for per_sample_pred, s in zip(batch_pred, reshaped_solution):
        gt_val = s[0]
        temp_list = per_sample_pred + [gt_val]  
        temp_tensor_list.append(temp_list)
        temp_tensor = torch.tensor(temp_list, dtype=torch.float32, device=device)
        sorted_indices = temp_tensor.argsort() 
        sorted_indices_list.append(sorted_indices)
        gt_sorted_idx = (sorted_indices == len(temp_list) - 1).nonzero().item()  
        for p in per_sample_pred:
            p_val = float(p)
            diff = abs(p_val - gt_val)

            if diff < 0.02:
                rank_reward = 1.0
            else:
                match_indices = (temp_tensor == p).nonzero(as_tuple=False)
                p_original_idx = match_indices[0].item()
                p_sorted_idx = (sorted_indices == p_original_idx).nonzero(as_tuple=False)[0].item()
                rank_diff = abs(p_sorted_idx - gt_sorted_idx)
                N_group = len(temp_list) 
                rank_reward = torch.exp(torch.tensor(-rank_diff / N_group, dtype=torch.float32, device=device))
            reward = torch.tensor(rank_reward, dtype=torch.float32, device=device)
            batch_rank_reward.append(reward)
    batch_rank_reward_tensor = torch.tensor(batch_rank_reward, dtype=torch.float32, device=device)
    lambda_rank = kwargs.get("lambda_rank", 1)# lambda
    batch_rank_reward_scaled = lambda_rank * batch_rank_reward_tensor
    rewards = [(r + float(rw))/3.0 for r, rw in zip(rewards, batch_rank_reward_scaled)]
    with open(log_path, "a", encoding='utf-8') as f:
        for i in range(len(reshaped_content)):
            f.write(f"------------- {current_time} batch std penalty: {len(batch_std_penalty[i])}    {batch_std_penalty[i]} -------------\n")
            f.write(f"------------- {current_time} batch triple compare: {len(batch_triple_compare)}     {batch_triple_compare[i]} -------------\n")

        f.write(f"------------- {current_time} temp_tensor_list: {len(temp_tensor_list)}      {temp_tensor_list} -------------\n")
        f.write(f"------------- {current_time} sorted_indices_list: {len(sorted_indices_list)}      {sorted_indices_list} -------------\n")
        f.write(f"------------- {current_time} batch rank rewards: {len(batch_rank_reward_scaled.tolist())}      {batch_rank_reward_scaled.tolist()} -------------\n")
        f.write(f"------------- {current_time} batch final rewards: {rewards} -------------\n")
        f.write(f"++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ {current_time} batch over ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
    return rewards
...
def main(script_args, training_args, model_args):
    ...
    all_data = []
    prompts = [
        "You are doing an image quality assessment. Provide five ratings in the following order: saturation, granularity, sharpness, foreground, and background. Please rate 5 scores each row. Each rating should be a float between 1 and 5, rounded to two decimals (1 = very poor, 5 = excellent).",
        "You are doing an image quality assessment. For this image quality assessment task, please evaluate along five aspects: color saturation, texture granularity, clarity or sharpness, foreground quality, and background quality. Please rate 5 scores each row. Give each aspect a score between 1.00 and 5.00 (two decimals, where 1 = very poor and 5 = excellent).",
        "You are doing an image quality assessment. Please assess image quality by providing five ratings in sequence: rate the saturation, rate the granularity, rate the sharpness, rate the foreground quality, and rate the background quality. Please rate 5 scores each row. All answers must be floats from 1.00 to 5.00, rounded to two decimals.",
        "You are doing an image quality assessment. Provide ratings for five dimensions: saturation to reflect color vividness, granularity to capture texture smoothness, sharpness to evaluate detail clarity, foreground quality to assess object visibility, and background quality to measure scene consistency. Please rate 5 scores each row. Each score must be a float between 1.00 and 5.00, where 1 means poor and 5 means excellent.",
        "You are doing an image quality assessment. Please give an image quality evaluation with five scores in the following order: saturation, granularity, sharpness, foreground quality, and background quality. Please rate 5 scores each row. Each score should be a float between 1 and 5, rounded to two decimals, with 1 indicating very poor quality and 5 indicating excellent quality."
    ]
    ...
    for data_file, image_folder, accu_reward_method in zip(data_files, image_folders, accu_reward_methods):
        with open(data_file, 'r') as f:
            for line in f:
                ...
                # item['problem'] = random.choice(prompts)
    ...
    # ---------- Load model & processor like inference ----------
