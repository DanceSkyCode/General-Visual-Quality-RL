...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 如果没指定 processing_class，自动加载一个合适的 processor/tokenizer。

        # 设置自定义处理参数，保证 padding / EOS 等 ID 正确。

        # 把这个 processor 绑定到主模型和 reference 模型上，使它们一致。
        ...
        print("reward_processing_classes:", reward_processing_classes)
        # Data collator
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # 使用 unwrap_model_for_generation 确保如果模型被包装过（如 DeepSpeed/Accelerate）也能正确调用 generate
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            # 调用模型生成输出，排除不参与生成的参数，并使用自定义的 generation_config
            ...
            # 获取 prompt 的长度（token 数量）
            prompt_length = prompt_ids.size(1)

            if not self.vlm_module.is_embeds_input():
                # 如果模型输入不是 embedding，而是 token id
                prompt_completion_ids = generate_returned_result  # prompt + completion 的 token
                prompt_ids = prompt_completion_ids[:, :prompt_length]  # 拿回原 prompt 部分
                completion_ids = prompt_completion_ids[:, prompt_length:]  # 拿出生成的 completion 部分
            else:
                # 如果输入是 embedding（例如图像+文本组合的 embedding）
                completion_ids = generate_returned_result  # 生成的 completion token
                prompt_completion_ids = torch.cat([prompt_ids, completion_ids], dim=1)  # 拼回完整序列

        # -------------------- EOS token 处理 --------------------
        # 找到每个生成序列中的 EOS token
        is_eos = completion_ids == self.processing_class.eos_token_id
        # 初始化每条序列的 EOS 索引为最大长度
        eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=device)
        # 如果有 EOS，更新对应索引为 EOS 第一次出现的位置
        eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)]
        # 为每个序列生成索引矩阵
        sequence_indices = torch.arange(is_eos.size(1), device=device).expand(is_eos.size(0), -1)
        # completion_mask：mask 在 EOS 之后的 token
        completion_mask = (sequence_indices <= eos_idx.unsqueeze(1)).int()

        # 将 prompt_mask 和 completion_mask 拼接，用于后续计算 logits
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)  # shape = (B, P+C)

        # -------------------- 多模态输入准备 --------------------
        multimodal_keywords = self.vlm_module.get_custom_multimodal_keywords()
        # 从 prompt_inputs 中提取多模态输入，例如图像 embedding
        multimodal_inputs = {k: prompt_inputs[k] if k in prompt_inputs else None for k in multimodal_keywords}

        # -------------------- 计算 token log-probabilities --------------------
        with torch.no_grad():  # 不计算梯度
            if self.num_iterations > 1:
                # 如果多次迭代，获取每个 token 的 log-prob
            ...
            # 根据 beta 和是否有 ref_model 决定是否计算 ref token log-prob
            if self.beta == 0.0:
                ...
                # 如果没有 ref_model，使用禁用 adapter 的模型本身作为参考
                with self.accelerator.unwrap_model(model).disable_adapter():
                    ref_per_token_logps = self._get_per_token_logps(
                        model, prompt_completion_ids, attention_mask, **multimodal_inputs
                    )

        # 如果计算了 ref token log-prob，只保留 completion 部分
        ...
        # 如果是对话场景，把生成内容封装成 role + content 的格式
        if is_conversational(inputs[0]):
            completions = [[{"role": "assistant", "content": completion}] for completion in completions]

        # -------------------- 计算奖励 --------------------
        # 初始化奖励矩阵，shape = (num_prompts, num_reward_funcs)
        rewards_per_func = torch.zeros(len(prompts), len(self.reward_funcs), device=device)

        # 遍历每个 reward model/function
        for i, (reward_func, reward_processing_class) in enumerate(
            zip(self.reward_funcs, self.reward_processing_classes)
        ):
            if isinstance(reward_func, PreTrainedModel):
                # 如果是对话，先把 prompts + completions 封装成 messages
                if is_conversational(inputs[0]):
                    messages = [{"messages": p + c} for p, c in zip(prompts, completions)]
                    texts = [apply_chat_template(x, reward_processing_class)["text"] for x in messages]
                else:
                    # 非对话，直接拼接 prompts + completions
        ...
        # -------------------- 返回结果 --------------------
        return {
            "prompt_ids": prompt_ids,  # 原始输入 prompt 的 token id
            "prompt_mask": prompt_mask,  # prompt 的 attention mask，1 表示有效 token，0 表示 padding
            "completion_ids": completion_ids,  # 模型生成的 completion token id
            "completion_mask": completion_mask,  # completion 的有效 mask，1 表示有效 token（到 EOS 位置），0 表示 padding
            "old_per_token_logps": old_per_token_logps,  # 之前迭代（或旧模型）计算的每个 token 的 log probability
            "ref_per_token_logps": ref_per_token_logps,  # 参考模型（ref_model）计算的每个 token 的 log probability
            "advantages": advantages,  # 归一化后的 reward advantages，用于强化学习更新
            "multimodal_inputs": multimodal_inputs  # 模型生成所需的额外多模态输入（如图像 embedding、视觉特征等）
        }
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 计算每个 token 的 log probability：这是 PPO 中新策略概率 log π(a|s)。

        # 只保留 completion 部分的 log probability，因为 prompt 不参与更新。
        ...
        # 优势函数 (advantages) 来衡量每个 token 的价值
